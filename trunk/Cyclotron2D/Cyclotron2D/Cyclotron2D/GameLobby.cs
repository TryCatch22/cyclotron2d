...
namespace Cyclotron2D {
	class GameLobby {
		...
		GameLobbyThread ClientAcceptThread;		//Demo thread to accept clients without blocking the application
		...
		public GameLobby() {
			...
			try {
				this.GameLobbySocket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
		...
		private void acceptClient() {
			...
			if (!this.isFull() && waitingForConnections) {
